...
import { Activity, ArrowLeft, LayoutDashboard, PanelLeft, Settings, Users, X } from "lucide-react";
...
const navItemsAdmin = [
  ...
  { to: "/admin/monitoring", key: "monitoring", icon: Activity },
...
function isPathActive(pathname: string, to: string) {
  ...
  if (to === "/admin/users" || to === "/admin/settings") return pathname === to;
...
export function AdminShell({
  ...
  const activePage = useMemo(() => {
    const found = navItemsAdmin.find((item) => isPathActive(location.pathname, item.to));
  ...
  return (
    <AnimatedSidebarProvider {...useSidebarOpen()} className="h-screen overflow-hidden">
      <AnimatedSidebar ariaLabel="Kumix Admin" collapsible="icon" className="min-h-0">
          ...
          <div className="flex min-h-11 items-center gap-3 overflow-hidden px-2">
            <Link to="/admin" className="flex items-center gap-2">
              ...
              <Badge
                ...
                radius="full"
                className="font-normal group-data-[state=collapsed]/sidebar:hidden"
              >
                {tNav("admin")}
              </Badge>
            </Link>
        ...
        <AnimatedSidebarContent className="px-2 pt-1">
          <AnimatedSidebarGroup className="pb-2">
            ...
            <AnimatedSidebarGroupContent>
              <AnimatedSidebarMenu>
                {navItemsAdmin.map(({ to, key, icon: Icon }) => (
