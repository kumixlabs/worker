 ...
 */
export function listTargets(userId?: string): TargetRecord[] {
  const sql = userId
 ...
 */
export function createTarget(input: TargetCreateInput, userId?: string | null): TargetRecord {
  const id = `tgt_${nanoid(12)}`;
  const now = nowIso();
  getDb()
    .query(
      "INSERT INTO targets (id, user_id, label, ingest_url, stream_key_cipher, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .run(
      id,
      userId ?? null,
