...
- API routes require Bearer token auth unless explicitly public. After password login (or handoff), the SPA stores the worker token in **sessionStorage** and sends it as Bearer. On 401 the SPA clears the session.
