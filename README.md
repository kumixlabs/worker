...
- Media library: direct file upload, direct URL import (SSRF-safe), and Google Drive imports into folders, with FFprobe metadata, thumbnails, and content-hash dedup.
- Probes media with FFprobe and validates H.264/AAC with max video bitrate 35 Mbps.
- Playlists combine videos plus an optional looping BGM audio track.
- Creates manual, scheduled, and recurring stream jobs to an RTMP target or a connected YouTube channel.
- Runs FFmpeg jobs and tracks runtime metrics, status, and logs.
- Recovers interrupted streams safely after restart and auto-resumes running streams on boot.
...
- Media page for uploading, importing from URL or Google Drive, folders, preview, rename, move, and delete.
- Playlists pages for building video + BGM playlists from library media.
- Streams page for stream lifecycle actions, logs, scheduling, and safe deletion.
...
- Direct file uploads (mp4, mp3, jpg, png) and direct http(s) URLs.
- Google Drive shared links with safe file ID parsing.
- SSRF protection with DNS resolution checks and per-redirect-hop validation so requests only reach vetted public addresses.
- Download size limits, quota enforcement, and cleanup on failure.
- FFprobe metadata extraction with a probe timeout.
- SHA-256 content dedup: identical uploads are rejected with 409 `duplicate_media`.
...
- `/api/media` (upload, import-url, import-gdrive, folders), `/api/playlists`, `/api/streams`
...
Media preview and log streaming endpoints are authorized through short-lived signed URLs so the dashboard `<video>` element can stream content directly.
...
Signed URL routes are generated by `POST /api/events/signed-url` and `POST /api/media/:id/preview-url`, and are short-lived.
...
- Tenancy: `media`, `media_folders`, `playlists`, `streams`, `events`, and `youtube_connections` are scoped to `user_id`; non-admin sessions can only touch their own rows on every route (including preview, export, SSE).
- Deleting a user cascades their media, playlists, streams, events, YouTube connections, and auth records.
