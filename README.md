...
Kumix Worker is the self-hosted Kumix live-stream runner. It provides a local dashboard, a token-protected HTTP API, persistent SQLite state, source download/probing, scheduling, FFmpeg execution, runtime monitoring, and a small public API surface for external integrations.
...
- Exposes monitoring and token rotation endpoints for external integrations.
...
Current test suite (99 tests) covers:
